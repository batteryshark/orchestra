...
`orchestra ui --tailscale` binds only to the machine's Tailscale IPv4 address. Anyone allowed by your tailnet ACLs to reach that host and port can view the registered projects' run metadata, prompts, transcripts, and logs, and can stop active runs from the details pane. Review those ACLs before enabling tailnet access.
