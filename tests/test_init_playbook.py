...
class PlaybookTemplateTests(unittest.TestCase):
    def test_packaged_template_contains_generic_coordination_contract(self) -> None:
        ...
        self.assertIn("reusable **launch profile**", template)
        self.assertIn("singleton worker", template)
        self.assertIn("zero, one, or several active", template)
        self.assertIn("profiles may draw from the same provider quota", template)
        self.assertIn("bare `send` targets the profile's shared inbox", template)
