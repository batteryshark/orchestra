...
from orchestra_cli import db, host, projects
...
def serve(root: Path, port: int = 4764, open_browser: bool = True) -> None:
    # `orchestra ui` is responsible for keeping the launch root in the
    # picker's allowlist — otherwise the user would have to run an
    # extra `orchestra project register` step before the dashboard
    # served anything. Auto-register best-effort; we never raise on a
    # registry problem (the handler still merges in the launch root
    # per-request as a fallback).
    try:
        projects.register(root)
    except projects.NotAnOrchestraRoot:
        pass  # find_root() already vetted this; defensive only
    except Exception:
        pass
