...
def after_report(con, cfg: dict, client: WorkClient, actions: list) -> None:
    ...
    for action in list(actions):
        ...
        try:
            ...
            print(f"dromond verify: {action['item']} failed: {exc}")
...
def sign_off(con, cfg: dict, client: WorkClient, item_id: str, worker_id: int,
    ...
    if not profile_name:
        print(f"dromond verify: {item_id} skipped — no verify_profile")
        return
    if profile_name == worker["profile"]:
        print(f"dromond verify: {item_id} skipped — verify_profile is the "
              f"worker's profile ({profile_name})")
        return
    try:
        profile = config.staff_profile(pcfg, profile_name)
    except SystemExit as exc:
        print(f"dromond verify: {item_id} not staffed: {exc}")
        return
    ...
    try:
        ...
        print(f"dromond verify: {item_id} aborted: {exc}")
...
def _insert(con, worker, root: Path, profile_name: str, profile: dict,
    ...
    raise RuntimeError("dromond: could not mint a unique verify-run slug")
